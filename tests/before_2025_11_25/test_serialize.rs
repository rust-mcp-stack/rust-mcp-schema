...
fn test_client_initialize_request() {
    ...
    assert!(matches!(message.request_id(), Some(request_id) if matches!(request_id , RequestId::Integer(r) if *r == 15)));
...
fn test_client_initialized_notification() {
    let init_notification = InitializedNotification::new(Some(InitializedNotificationParams { meta: None, extra: None }));

    let message: ClientMessage = ClientMessage::Notification(ClientJsonrpcNotification::new(
        NotificationFromClient::ClientNotification(ClientNotification::InitializedNotification(init_notification.clone())),
    ));
...
fn test_server_cancel_notification() {
    ...
    let message: ServerMessage = ServerMessage::Notification(ServerJsonrpcNotification::new(
        NotificationFromServer::ServerNotification(ServerNotification::CancelledNotification(cancel_notification.clone())),
    ));
